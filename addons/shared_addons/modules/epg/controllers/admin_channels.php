	...
	public function create(){
		...
		if($this->form_validation->run()){
			...
			if($this->epg_ch_m->add_channel($data)){
				...
				$this->render('admin/channel_form', array('page'=>$this->page_data));
			}
		}else{						$this->ch_data = $this->epg_ch_m->add_new();
			$temp = $this->epg_ch_m->select('cat')->get_categories();			foreach($temp as $key=>$cat){				$this->ch_data->categories[] = $cat->cat;			}						$this->render('admin/channel_form', array('page'=>$this->page_data, 'ch'=>$this->ch_data));
		}
	}
